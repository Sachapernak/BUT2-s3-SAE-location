...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('Bail correctement ajoute');
END;
/
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('Mise a jour reussie pour bail_1.');
END;
/
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('Mise a jour echouee, bail_inexistant.');
END;
/

--------- Test 6 DELETE bail ------------
BEGIN
    DBMS_OUTPUT.PUT_LINE('Test 6 delete bail');
    SAE_DAO_BAIL.SAE_DELETE('bail_1');
    select * from sae_bail where id_bail = 'bail_1';
    DBMS_OUTPUT.PUT_LINE('Bail correctement supprime');
END;
/
Rollback;
